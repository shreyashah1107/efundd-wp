<section class="corporate-content-section--nine">
  <div class="content-wrapper">
    <div class="container">
      <div class="row justify-content-center">
        <div class="col-xxl-11">
          <div class="row align-items-center justify-content-center justify-content-lg-start">
            <div class="col-sm-10 col-md-8 col-lg-6 col-xl-6">
              <div class="image wow fadeInLeft" data-wow-delay="0.1s">
                <img src="<?php echo get_template_directory_uri(); ?>/assets/img/redesign/corporate/eight.png" alt="Image">
              </div>
            </div>

            <div class="col-lg-6 col-xl-6 mt-5 mt-lg-0 p-lg-0 ps-lg-4">
              <div class="title-row mb-5">
                <h3 class="section-title section-title--xl text-anime-style-3" data-cursor="-opaque">Manage your global payments</h3>
              </div>

              <div class="payment-list--items">
                <div class="payment-list--item wow fadeInUp" data-wow-delay="0.1s">
                  <h4>Create account within minutes</h4>
                  <p>Set up your Paayco account quickly with our simple onboarding process.</p>
                </div>

                <div class="payment-list--item wow fadeInUp" data-wow-delay="0.2s">
                  <h4>Get Instant local and International accounts details</h4>
                  <p>Get instant bank details for local transactions and IBAN for all international transactions</p>
                </div>

                <div class="payment-list--item wow fadeInUp" data-wow-delay="0.3s">
                  <h4>Instant virtual Mastercard</h4>
                  <p>Quickly receive a virtual Mastercard for to get paid.</p>
                </div>

                <div class="payment-list--item wow fadeInUp" data-wow-delay="0.4s">
                  <h4>Transact in 140+ countries and 20+ currencies</h4>
                  <p>Use your Mastercard in anywhere, anytime </p>
                </div>
              </div>

              <div class="btn-grp mt-5">
                <a href="#" title="Explore API docs" class="btn btn-bright-green wow fadeInUp" data-wow-delay="0.2s">Explore API docs</a>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</section>